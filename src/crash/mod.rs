#[allow(dead_code)]
pub fn hash_calc(message: &str) -> String {
    // Initialization
    let word_size = 16;
...
impl Shade {
    pub fn hash(input: &str, dlen: u32) -> String {
        ...
        // Create words
        let stage1_word_chunks: Vec<_> = msg_bytes.chunks(word_size).collect();
        let mut stage2_words: Vec<u64> = vec![];
        let stage1_words: Vec<&u8> = stage1_word_chunks.into_iter().flatten().collect();
        // Operations
        for (count, stage1_word) in stage1_words.clone().into_iter().enumerate() {
            ...
            interlaced = (85331_u64.wrapping_mul(interlaced))
                .wrapping_shr(word_size.wrapping_sub(16) as u32);
            stage2_words.push(interlaced);
        }

        // Truncate and formatting
        let mut accumulator: String = "".to_string();
        for item in stage1_words {
            accumulator = format!("{accumulator}{:x}", item);
        }
